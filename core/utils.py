...
def validate_inputs(start_date, end_date):
    ...
    assert end_date - start_date >= timedelta(
        hours=1
    ), "Minimum interval has to be 1 hour"

    # Assert within records
    assert RECORDS_START <= start_date, f"Earliest date has to be {RECORDS_START}"
    assert (
        end_date <= maximum_date_end_exclusive()
    ), f"Latest end date has to be {maximum_date_end_exclusive()}"
...
def resample_to_standard_granularity(
    df: pd.DataFrame, granularity: timedelta
) -> pd.DataFrame:
    if df.empty:
        return df
...
def apply_to_fields(data: Data, func: Callable[[pd.DataFrame], pd.DataFrame]):
